--[[ 
    申请合并队伍item 逻辑  包括列表，展示，关闭
]]
local class_name = "FriendTeamMergeRequestItem"
local FriendTeamMergeRequestItem = BaseClass(nil, class_name)


function FriendTeamMergeRequestItem:OnInit()
    self.BindNodes = {
...
function FriendTeamMergeRequestItem:OnShow(Param)
    self.InnerItemList = {}
    self.Param = Param
    self:UpdateListShow()
end

function FriendTeamMergeRequestItem:OnHide()
    self.InnerItemList = {}
    self.View.RequestListBox:ClearChildren()
end

function FriendTeamMergeRequestItem:UpdateListShow()
    -- self.View.LbTitleName:SetText(StringUtil.Format("合并队伍"))
    ...
    for Index,Show in ipairs(ShowList) do
        ...
        if not (Item and CommonUtil.IsValid(Item.View)) then
            ...
            Item = UIHandler.New(self,Widget,require("Client.Modules.Friend.FriendItems.Inner.FriendTeamMergeRequestItemInner"),Param).ViewInstance
...
function FriendTeamMergeRequestItem:DoAnimation(IsOpen)
    for i, Item in ipairs(self.InnerItemList) do
        self:DoOneItemAnimation(Item, IsOpen, i)
    end
end

function FriendTeamMergeRequestItem:DoOneItemAnimation(Item, IsOpen, Index)
    if Item.OnListStateChange then
        Item:OnListStateChange(IsOpen, Index)
    end
end

function FriendTeamMergeRequestItem:OnClick_GUIButton_Open()
    --展开
...
function FriendTeamMergeRequestItem:OnClick_GUIButton_Hide()
    --隐藏
    self.ListOpenState = false
    self:UpdateListShow()
...
function FriendTeamMergeRequestItem:CheckTeamState()
...
return FriendTeamMergeRequestItem
